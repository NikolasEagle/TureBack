...
backuping() {
    ...
    if [ $count_backup_files_in_folder -ge $count_backup_files ]; then
        find $backup_path -name "*.img.lz4" -exec ls -t {} + | tail -n $(($count_backup_files_in_folder - $count_backup_files + 1)) | xargs rm
        if [ $? -ne 0 ]; then
    ...
    if [ $? -ne 0 ]; then
        ...
        find $backup_path -name "*.img.lz4" -exec ls -t {} + | head -n 1 | xargs rm
        if [ $? -ne 0 ]; then
